...
class HigherStudies extends React.Component {
    ...
    componentDidMount() {
        ...
        this.props.dispatch(getProductByCategory(data)).then((response) => {
            if (response && response.data.length > 0) {
                this.setState({ higherStudies_data: response.data });
            }
        });
    }
    ...
    render() {
        return (
            <>
                <section className="bg0 pt-lg-3 pb-lg-3 overflow-hidden higher-school-block">
                    <div className="container">
                        <div className="row">
                            ...
                            <h3 className="section_title mb-lg-4 mb-3">Universities & Colleges </h3>
                               ...
                               <i className="ml-2 fa fa-angle-right fs-14 font-weight-bold" aria-hidden="true"></i></a>
                                ...
                                <Carousel responsive={responsive}>
                                     ...
                                     {this.state.higherStudies_data && this.state.higherStudies_data !== undefined && this.state.higherStudies_data.length > 0?(
                                        this.state.higherStudies_data.map((item, index) => (
                                     <div key={'item' + index} className="cursor-point p-3 item d-flex flex-column bg-pro rounded-4 electric">
                                                <div className="d-flex align-items-baseline justify-content-end">
                                                    
                                                    <button type="button"  onClick={() => this.setFavourite(item.sfid, `fav-school-item-${item.id}`)} id={`fav-school-item-${item.id}`} className={`wish text-right mt-2 ${item.isFavorite?'active':''}`}><i className="fa fa-heart-o" aria-hidden="true"></i></button>
                                                </div>
                                                <div>
                                                ...
                                                <p className="mb-2">{item.name.slice(0, 15)}</p>
                                                    {item.no_cost_emi__c && <h4>
                                                        No Cost EMI
                                                        <span className="d-block">Starting at₹{item.item.min_avi_emi__c} </span>
                                                    </h4>}
                                                </div>
                                                </div>
                                     </div>
