...
class ProductTablet extends React.Component {
    ...
    componentDidMount() {
        ...
        this.props.dispatch(getProductByCategory(data)).then((response) => {
            if (response && response.data.length > 0) {
                this.setState({ tablets_data: response.data });
            }
        });
    }
    ...
    render() {
        return (
            <>
                <section className="bg0 pt-lg-3 pb-lg-3 overflow-hidden tablet-deal-block">
                    <div className="container">
                        <div className="row">
                            <div className="col mb-2 tablets">
                            <div className="d-flex justify-content-between align-items-center mb-2 shopby-after-bg-image position-rel">
                            <h3 className="section_title mb-lg-4 mb-3">Best Deals on Tablets</h3>
                               ...
                               <i className="ml-2 fa fa-angle-right fs-14 font-weight-bold" aria-hidden="true"></i></a>
                                ...
                                <Carousel responsive={responsive}>
                                     ...
                                     {this.state.tablets_data && this.state.tablets_data !== undefined && this.state.tablets_data.length > 0?(
                                        this.state.tablets_data.map((item, index) => (
                                     <div key={'item' + index} className="cursor-point p-3 item d-flex flex-column bg-pro rounded-4 electric">
                                                <div className="d-flex align-items-baseline justify-content-end">
                                                    
                                                    <button type="button" onClick={() => this.setFavourite(item.sfid, `fav-tab-item-${item.id}`)} id={`fav-tab-item-${item.id}`} className={`wish text-right mt-2 ${item.isFavorite?'active':''}`}><i className="fa fa-heart-o" aria-hidden="true"></i></button>
                                                </div>
