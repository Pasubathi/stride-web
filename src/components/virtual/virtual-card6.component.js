...
class VirtualCard6 extends Component {
    ...
    async componentDidMount() {
       const { dispatch, user, selectedplan, sfid } = this.props
      ...
      $('.label input').change(function(){
        ...
        dispatch(getBanks());
        dispatch(getAccountProfile(acObj)).then((response)=>{
            if(response.status ==="success")
            {
                this.setState({mobile: response.accountDet.phone});
            }
        });
        dispatch(getPreferred(data)).then((response)=>{
            if(response.status ==="success")
            {
        ...
        if(!process.env.NODE_ENV || process.env.NODE_ENV === 'development') {
            const redirectUrl = process.env.REACT_APP_USER_VIRTUAL_DEV_PAYMENT_URL;
            this.setState({ REDIRECT_URL: redirectUrl});
          } else {
            const redirectUrl = process.env.REACT_APP_USER_VIRTUAL_LIVE_PAYMENT_URL;
            this.setState({ REDIRECT_URL: redirectUrl});
          }
    }
    ...
    handleCardSubmit = () =>{
        ...
        const { card_cvv, card_expiry, card_name, card_number, isSave } = this.state
        let data = {
    ...
    renderBanks = (banks, selectedBank) =>{
        ...
        banks.map((items, index)=>{
            row.push(
                <li onClick={() => this.handleBankSelect(items.bank_id)} key={`bank-${index}`} className={`bank d-flex align-items-center justify-content-center flex-column cursor-point ${items.bank_id === selectedBank?"active":""}`}>
    ...
    render() {
        ...
        const { cards, selectedBank } = this.state
                   ...
                   <div className='row'>
                       <div className='col-sm-12'>
                           <div className='d-flex align-items-center'>
                               <button onChange={this.handleBack} type='button' className='back-btn rounded-circle mr-3 mr-lg-4'>
                                   back
                               </button>
                                    ...
                                    <div className='col-lg-7 pl-lg-0'>
                                        <div className='position-relative p_m-tabs_content_wrapper'>
                                            <div className='p_m-tabs_content' id="1">
                                                ...
                                                {/* <div className='pp-info border py-3 py-lg-4 px-lg-5 px-3 mt-lg-5 mt-4 rounded-lg'>
                                                    <div className='pp_info_row d-flex flex-wrap mb-4'>
                                                        <div className='left'>
                                                        <span><img src={asset+"images/mastercard2x.png"} alt="mc" /></span>
                                                        </div>
                                                        <div className='right'>
                                                            <p className='m-0 cn'>5305-XXXXXXXX-7696</p>
                                                            <p className='m-0 ed'>CREDIT CARD | VALID TILL 09/24</p>
                                                        </div>
                                                    </div>
                                                    <div className='pp_info_row d-flex flex-wrap'>
                                                        <div className='left'>
                                                            <input type="text" className='cvv-input' placeholder='CVV'/>
                                                        </div>
                                                        <div className='right'>
                                                            <div className='emi-amount d-flex align-items-center justify-content-center'>Pay <i className='rupee'>`</i> {this.state.downpayment?this.state.downpayment.toLocaleString('en-IN'):''}</div>
                                                        </div>
                                                    </div>
                                                </div> */}
                                            </div>
                                    ...
                                    data-target="#howItWork">How it works</button>
                                                ...
                                                <div className='mt-lg-5 mt-4 card_detail_form'>
                                                    ...
                                                    <div className='card_detail_form'>
                                                        ...
                                                        <div className='vpa-save-check mb-4'>
                                                            <div className="custom-control custom-checkbox mr-sm-2">
                                                            <input type="checkbox" name="isSave" defaultChecked={this.state.isSave} onChange={this.handleCheckbox} className="custom-control-input" id="customControlAutosizing2"/>
                                                            <label className="custom-control-label" htmlFor="customControlAutosizing2"> Securely save this card for a faster checkout next time.</label>
                                                        </div>
                                                        </div>
                                                        <div>
                                                        <button 
                                            ...
                                            <div className='p_m-tabs_content' id="4">
                                                ...
                                                <div className='other-bank position-relative mb-4'>
                                                    <select name='selectedBank' value={selectedBank?selectedBank:''} onChange={this.handleChange}>
                                                        <option value="">Other bank</option>
                                                        { banks && banks.length > 0 && banks.map((items, index)=>(
                                                            <option value={items.bank_id}  key={`bank-options-${index}`}>{items.bank_name}</option>
                                                        ))}
                                                    </select>
                                                </div>
                                                <div>
...
export default connect(mapStateToProps)(VirtualCard6)